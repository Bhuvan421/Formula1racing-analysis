...
circuits_df = spark.read.csv(f'{raw_folder}/{v_file_date}/circuits.csv')
...
circuits_df = spark.read \
            .format('csv') \
            .load(f'{raw_folder}/{v_file_date}/circuits.csv')
...
circuits_df.write.mode('overwrite').format('delta').saveAsTable('f1_processed.circuits_cleaned')
