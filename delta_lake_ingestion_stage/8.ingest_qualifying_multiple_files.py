...
display(dbutils.fs.ls(f'/mnt/formula1racingstoragedl/raw/{v_file_date}/qualifying/'))
...
qualifying_df = spark.read \
        .format('json') \
        .option('multiLine', True) \
        .load(f'{raw_folder}/{v_file_date}/qualifying/')
...
qualifying_df = spark.read \
        .format('json') \
        .option('multiLine', True) \
        .load(f'{raw_folder}/{v_file_date}/qualifying/qualifying_split*.json')
...
# overwrite_partition(qualifying_df, 'f1_processed', 'qualifying_cleaned', 'race_id')
...
merge_condition = "targetTable.qualify_id = sourceTable.qualify_id AND targetTable.race_id = sourceTable.race_id"
merge_delta_data(qualifying_df, 'f1_processed', 'qualifying_cleaned', processed_folder, merge_condition, 'race_id')
...
# MAGIC from f1_processed.qualifying_cleaned
