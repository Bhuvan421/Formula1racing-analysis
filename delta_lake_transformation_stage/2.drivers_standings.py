...
race_years = spark.read.format('delta').load(f'{presentation_folder}/race_results') \
...
df = race_results_df.groupBy('race_year', 'driver_name', 'driver_nationality') \
...
# final_df.write.mode('overwrite').parquet(f'{presentation_folder}/drivers_standings')
...
merge_condition = "targetTable.driver_name = sourceTable.driver_name AND targetTable.race_year = sourceTable.race_year"
merge_delta_data(final_df, 'f1_presentation', 'drivers_standings', presentation_folder, merge_condition, 'race_year')
...
display(dbutils.fs.ls(f'{presentation_folder}/drivers_standings'))
